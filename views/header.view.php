...
<body>
                ...
                <div class="hidden w-full md:block md:w-auto pr-8" id="navbar-default">
                    <ul class="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                        ...
                        if(!empty($_SESSION['user_id'])){
                            if(sessionIsValid($_SESSION['current_session'], $_SESSION['user_id'])){
                                $link="dc&session_id=".$_SESSION['current_session'];
                            }else{
                                $link="dc&session_id=new";
                            }
                        }else{
                            $link="register";
                        }
                        ?>                           
                        <a href="index.php?page=<?= $link?>" class="block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:border-0 md:hover:text-blue-700 md:p-0 dark:text-white md:dark:hover:text-blue-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent">
    ...
    <?php 
    if(sessionIsValid($_SESSION['current_session'], $_SESSION['user_id'])){
        if($_GET['page'] == 'dc' && !empty($_GET['session_id']) ){
            $nbRep = getProgress($_SESSION['current_session'], false);
    ?>
        <section id="stepper" class="flex justify-center flex-col bg-slate-100 md:w-[460px] lg:w-[780px] xl:w-[1050px] m-auto p-8 mt-2">
                <div class="w-full justify-center dark:bg-gray-700 pb-8 ">        
                        <h1>Progression (<?= $nbRep?> / 133)</h1>
                </div>
                <div class="w-full h-4 mb-4 bg-gray-200 rounded-full dark:bg-gray-700">
                    <div class="h-6 flex   justify-center bg-blue-600 rounded-full dark:bg-blue-500" style="width:<?=$_SESSION['progress_value'];?>%"><p class="text-white font-medium"><?=$_SESSION['progress_value'];?>%</p></div>
                    </div>
                </div>
        </section>    
    <?php 
